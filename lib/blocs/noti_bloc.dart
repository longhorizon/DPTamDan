...
class NotiScreenBloc extends Bloc<NotiScreenEvent, NotiScreenState> {
  NotiScreenBloc() : super(InitialState());

  @override
  Stream<NotiScreenState> mapEventToState(
    NotiScreenEvent event,
  ) async* {
  ...
  Stream<NotiScreenState> _mapFetchDataToState() async* {
    yield LoadingState();
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      String uid = prefs.getString('user_id').toString();
      String token = prefs.getString('token').toString();
      var uri = Uri.https(Constants.apiUrl, 'api/client/notification');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 200) {
          final List<NotificationData> notis = (data['result'] as List<dynamic>)
